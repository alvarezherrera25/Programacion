...
# CONFIGURACIÓN
# -----------------------------------------------------------------------------
MODO_ENTRENAMIENTO = True # TRUE = COLOR TRAINER
...
class DetectorIA:
    def __init__(self):
        ...
        print("[SISTEMA] MODO ARDUINO TRAINER (Colores) ACTIVADO")
        print("Detectando objetivos: AZUL, NARANJA, ROJO")
    ...
    def _inference_loop(self):
        """Hilo que corre la IA de colores"""
        while self.running:
            img_to_process = None
            with self.lock:
    ...
    def es_enemigo_por_color(self, img_roi):
        # En modo entrenamiento siempre validamos
        return True
    ...
    def procesar_frame(self, debug=False):
        ...
        if debug:
            ...
            estado = "ARDUINO TRAINER" if EJECUTANDO else "ESPERA"
        ...
        if debug and annotated_frame is not None:
            cv2.imshow("ARDUINO TRAINER VISION", annotated_frame)
            cv2.waitKey(1)
    ...
    while PROGRAMA_ACTIVO:
        try:
            ...
            if EJECUTANDO and encontrado and (time.time() - ultimo_disparo > cooldown):
                disparo_tactico()
                print(f"[HW DISPARO!] Objetivo marcado") 
...
def iniciar():
    ...
    print("=== AUTO TRIGGER ARDUINO - TRAINER ===")
